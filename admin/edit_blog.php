...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $db_image_path = $blog['image_path']; // Default to old image
    
    // Validate
    if (empty($title) || empty($content)) {
        $_SESSION['error'] = "يرجى ملء الحقول المطلوبة.";
    } else {
        // Image Upload if checked
        if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($_FILES['image']['tmp_name'], $target_path)) {
                ...
                if (!empty($blog['image_path']) && strpos($blog['image_path'], 'http') !== 0) {
                    $old_file = '../' . $blog['image_path'];
                ...
                $_SESSION['error'] = "فشل في رفع الصورة الجديدة.";
            }
        }
        
        if (!isset($_SESSION['error'])) {
            try {
                $stmt = $pdo->prepare("UPDATE blogs SET title = ?, author = ?, excerpt = ?, content = ?, image_path = ?, created_at = ? WHERE id = ?");
                $stmt->execute([$title, $author, $excerpt, $content, $db_image_path, $created_at, $id]);
...
<head>
    ...
    <title>تعديل المقال | أوتو لاين</title>
            ...
            <div class="max-w-4xl mx-auto">
                ...
                <header class="mb-12">
                    <div class="flex items-center gap-4 mb-4">
                        ...
                        <p class="text-[10px] font-black uppercase tracking-[0.2em] text-[#c9a96e]">تعديل المقال</p>
                    </div>
                    <h1 class="text-4xl font-black text-white tracking-tight flex items-center gap-3">
                        <span class="material-symbols-outlined text-blue-500 text-4xl">edit_document</span>
                        تحديث المقال
                    </h1>
                </header>
                ...
                <div class="glass-card rounded-[2rem] p-8 md:p-12">
                    <form action="" method="POST" enctype="multipart/form-data" class="space-y-8" onsubmit="return handleFormSubmit(event)">
                        ...
                        <!-- Image -->
                        <div class="space-y-3">
                            <label class="block text-xs font-black text-slate-400 uppercase tracking-widest">صورة المقال (اتركه فارغاً للاحتفاظ بالصورة الحالية)</label>
                            
                            <!-- Current Image Preview -->
                            <?php if(!empty($blog['image_path'])): ?>
                                <div class="mb-4">
                                    <p class="text-sm text-slate-400 mb-2">الصورة الحالية:</p>
                                    <img src="../<?= htmlspecialchars($blog['image_path']) ?>" alt="Current Image" class="h-32 rounded-xl object-cover border border-white/10">
                                </div>
                            <?php endif; ?>
                            ...
                            <div class="border-2 border-dashed border-white/10 rounded-2xl p-8 text-center hover:border-[#c9a96e]/50 transition-colors bg-[#12110f]">
                                ...
                                <label for="image" class="cursor-pointer flex flex-col items-center gap-4">
                                    <div class="w-16 h-16 rounded-full bg-blue-500/10 text-blue-500 flex items-center justify-center">
                                        <span class="material-symbols-outlined text-2xl">change_circle</span>
                                    </div>
                                    <div>
                                        <p class="text-white font-bold text-lg mb-1">اضغط لاختيار صورة جديدة</p>
                                        <p class="text-sm text-slate-500">JPG, PNG, WebP</p>
                                    </div>
                                </label>
                                <div id="image-preview" class="mt-6 hidden">
                                    <p class="text-sm text-[#c9a96e] font-bold mb-2">الصورة الجديدة المُختارة:</p>
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="relative">
                                ...
                                <input type="datetime-local" name="created_at" value="<?= date('Y-m-d\TH:i', strtotime($blog['created_at'])) ?>"
                                    class="w-full bg-[#12110f] border border-white/10 rounded-xl text-white py-4 pl-4 pr-12 focus:outline-none focus:border-[#c9a96e] focus:ring-1 focus:ring-[#c9a96e] transition-all placeholder-slate-600">
                            </div>
                        </div>
                        ...
                        <div class="space-y-3">
                            ...
                            <textarea name="content" rows="15" required
                                class="w-full bg-[#12110f] border border-white/10 rounded-xl text-white p-4 focus:outline-none focus:border-[#c9a96e] focus:ring-1 focus:ring-[#c9a96e] transition-all placeholder-slate-600"><?= htmlspecialchars($blog['content']) ?></textarea>
                            <p class="text-xs text-slate-500 mt-2">* يدعم إدخال وسوم HTML</p>
                        </div>
